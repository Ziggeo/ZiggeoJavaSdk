...
public class ZiggeoEffectProfileProcess {
    ...
    if (file != null) {
        ...
        result = this.application.connect().postJSON("/v1/effects/" + effectTokenOrKey + "/process/" + result.get("token") + "/confirm-watermark");
        return result;
    } else
    ...
    if (file != null) {
        JSONObject result = this.application.connect().postUploadJSON("/v1/effects/" + effectTokenOrKey + "/process/" + tokenOrKey + "/watermark-upload-url", "effect_process", data, file, null);
        result = this.application.connect().postJSON("/v1/effects/" + effectTokenOrKey + "/process/" + tokenOrKey + "/confirm-watermark");
        return result;
    } else
